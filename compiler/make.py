...
parser.add_argument('--compile-cxx', '-c', action='store_true',
                    help='Compile the CXX codes and generate nugget.exe into built.')
parser.add_argument('--make-nri', '-n', action='store_true',
                    help='Generate nuggets.nri.')
parser.add_argument('--resources', '-m', action='store_true',
                    help='Pack resources.mf.')
...
class SamplePackager(NiraiPackager):
    ...
    def generate_niraidata(self):
        ...
        config = self.get_file_contents(self.BASEDIR + 'config' + os.sep + 'release.prc', True)
...
if args.compile_cxx:
    ...
    compiler.add_source('engine/core.cxx')
    compiler.run()

if args.make_nri:
    pkg = SamplePackager('../built/nuggets.nri')
...
if args.resources:
    os.chdir('../')
    cmd = 'multify -cf built/resources.mf resources'
